...
#include "Graphics/Private/ModelData.h"
...
namespace dy::Graphics
{
	...
	static bool LoadModel(const std::string& path, ModelData& outModel, const ModelLoadOptions& options)
	{
	...
	bool AddModelToScene(Scene& scene, const ModelSceneDesc& desc)
	{
		...
		for(const ModelMesh& modelMesh : model.meshes)
		{
			...
			const MaterialID material = modelMesh.materialIndex < materials.size()
				...
				: materials.front();
			(void)scene.CreateEntity(mesh, material, transform, desc.renderFlags);
			createdAny = true;
		}
		return createdAny;
	}
