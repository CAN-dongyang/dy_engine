...
namespace dy::Graphics
{
	namespace
	{
			...
			Math::Bounds3& bounds)
		{
			...
			const float tangent = std::tan(std::clamp(camera.fovYRadians * 0.5f, 0.05f, 1.5f));
		...
		float boundsPadding)
	{
		...
		cascades.splits = ComputePracticalCascadeSplits(camera.nearPlane, camera.farPlane, cascades.count, splitLambda);
