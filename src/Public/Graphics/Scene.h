...
namespace dy::Graphics
{
	...
	public:
		...
		[[nodiscard]] const TextureAsset& GetTexture(TextureID textureId) const { return m_textures[ToIndex(textureId)]; }
		[[nodiscard]] const MaterialDesc& GetMaterial(MaterialID materialId) const { return m_materials[ToIndex(materialId)]; }
		void SetMaterial(MaterialID materialId, const MaterialDesc& material) { m_materials[ToIndex(materialId)] = material; }
		[[nodiscard]] const MeshData& GetMesh(MeshID meshId) const { return m_meshes[ToIndex(meshId)]; }
		[[nodiscard]] MeshID GetEntityMesh(EntityID entityId) const { return m_entityMeshes[ToIndex(entityId)]; }
		[[nodiscard]] MaterialID GetEntityMaterial(EntityID entityId) const { return m_entityMaterials[ToIndex(entityId)]; }
		[[nodiscard]] const Transform& GetTransform(EntityID entityId) const { return m_entityTransforms[ToIndex(entityId)]; }
		[[nodiscard]] Transform& GetTransform(EntityID entityId) { return m_entityTransforms[ToIndex(entityId)]; }
		[[nodiscard]] const RenderFlags& GetRenderFlags(EntityID entityId) const { return m_entityRenderFlags[ToIndex(entityId)]; }
		void SetRenderFlags(EntityID entityId, const RenderFlags& renderFlags) { m_entityRenderFlags[ToIndex(entityId)] = renderFlags; }
		[[nodiscard]] const DirectionalLight& GetDirectionalLight(DirectionalLightID lightId) const { return m_directionalLights[ToIndex(lightId)]; }
		void SetDirectionalLight(DirectionalLightID lightId, const DirectionalLight& light) { m_directionalLights[ToIndex(lightId)] = light; }
		[[nodiscard]] const PointLight& GetPointLight(PointLightID lightId) const { return m_pointLights[ToIndex(lightId)]; }
		void SetPointLight(PointLightID lightId, const PointLight& light) { m_pointLights[ToIndex(lightId)] = light; }
